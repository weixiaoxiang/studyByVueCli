...
export default {
  ...
  mounted() {
    ...
    this.buildingHighlight = this.scene.primitives.add(
    ...
    this.treeHighlight1 = this.scene.primitives.add(
      new Cesium.ClassificationPrimitive({
        geometryInstances: new Cesium.GeometryInstance({
    ...
    this.treeHighlight2 = this.scene.primitives.add(
      new Cesium.ClassificationPrimitive({
        geometryInstances: new Cesium.GeometryInstance({
          geometry: new Cesium.EllipsoidGeometry({
            radii: new Cesium.Cartesian3(3.25, 5.0, 4.0)
    ...
    modelMatrix = Cesium.Transforms.eastNorthUpToFixedFrame(center)
    var translation = Cesium.Matrix4.fromTranslation(
      new Cesium.Cartesian3(0.0, 0.0, -2.0)
    )
    Cesium.Matrix4.multiply(modelMatrix, translation, modelMatrix)

    this.treeHighlight3 = this.scene.primitives.add(
      new Cesium.ClassificationPrimitive({
        geometryInstances: new Cesium.GeometryInstance({
          geometry: new Cesium.EllipsoidGeometry({
    ...
    this.treeHighlight4 = this.scene.primitives.add(
      new Cesium.ClassificationPrimitive({
        geometryInstances: new Cesium.GeometryInstance({
    ...
    handler.setInputAction((movement) => {
      ...
      if (
        ...
        if (!this.scene.invertClassification) {
  ...
  methods: {
    changeActiveTitle() {
      this.$store.commit('changeActiveTitle', 'classification的使用')
    },
    highlightBuilding() {
      this.camera.setView({
    ...
    invertClassification(checked) {
      if (checked && !this.scene.invertClassificationSupported) {
        window.alert('This browser does not support invert classification')
      }

      this.scene.invertClassification = checked
      this.scene.invertClassificationColor = new Cesium.Color(
        0.25,
        0.25,
        0.25,
        1.0
      ...
      this.buildingHighlight.getGeometryInstanceAttributes('volume').show =
        Cesium.ShowGeometryInstanceAttribute.toValue(!checked)
      this.treeHighlight1.getGeometryInstanceAttributes('volume 1').show =
        Cesium.ShowGeometryInstanceAttribute.toValue(!checked)
      this.treeHighlight2.getGeometryInstanceAttributes('volume 2').show =
        Cesium.ShowGeometryInstanceAttribute.toValue(!checked)
      this.treeHighlight3.getGeometryInstanceAttributes('volume 3').show =
        Cesium.ShowGeometryInstanceAttribute.toValue(!checked)
      this.treeHighlight4.getGeometryInstanceAttributes('volume 4').show =
        Cesium.ShowGeometryInstanceAttribute.toValue(!checked)
    },
    updateAlpha(value) {
      this.scene.invertClassificationColor.alpha = parseFloat(value)
    }
  }
}
</script>
