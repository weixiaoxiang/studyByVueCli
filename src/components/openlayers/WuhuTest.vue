...
export default {
  ...
  mounted() {
    ...
    this.$notify({
      title: '警告',
      message: '需要开启本机geoserver和postgresql服务,最短路径-紫色,道路类型-橙色,红绿灯-绿色',
      type: 'warning',
  ...
  methods: {
    initOlMap() {
      this.view = new View({
        ...
        center: [118.374658305, 31.329940991], // 芜湖
        zoom: 10,
        minZoom: 3,
        maxZoom: 20,
      ...
      const wuhulayer = new ImageLayer({
        source: new ImageWMS({
          ratio: 1,
          url: 'http://localhost:8083/geoserver/whtest/wms',
      ...
      this.map.on('singleclick', event => {
        if (this.startPoint.getGeometry() == null) {
          ...
          // 长度
          this.result1 = new ImageLayer({
            source: new ImageWMS({
              url: 'http://localhost:8083/geoserver/whtest/wms',
              params: {
                LAYERS: 'whtest:whtest1',
                FORMAT: 'image/png',
                VERSION: '1.1.1',
                SRS: 'EPSG:4326',
                STYLE: 'wuhu:line-wuhu1',
                viewparams
              }
            })
          })
          this.map.addLayer(this.result1)
          // 道路
          this.result2 = new ImageLayer({
            source: new ImageWMS({
              url: 'http://localhost:8083/geoserver/whtest/wms',
              params: {
                LAYERS: 'whtest:whtest2',
                FORMAT: 'image/png',
                VERSION: '1.1.1',
                SRS: 'EPSG:4326',
                STYLE: 'wuhu:line-wuhu2',
                viewparams
              }
            })
          })
          this.map.addLayer(this.result2)
          // 红绿灯
          this.result3 = new ImageLayer({
            source: new ImageWMS({
              url: 'http://localhost:8083/geoserver/whtest/wms',
              params: {
                LAYERS: 'whtest:whtest3',
                FORMAT: 'image/png',
                VERSION: '1.1.1',
                SRS: 'EPSG:4326',
                STYLE: 'wuhu:line-wuhu3',
                viewparams
              }
            })
          })
          this.map.addLayer(this.result3)
        }
      })
    },
    // 清除路径
    remove() {
      ...
      // 移除结果路径图层
      this.map.removeLayer(this.result1)
      this.map.removeLayer(this.result2)
      this.map.removeLayer(this.result3)
    },
    changeActiveTitle() {
      // https://zhuanlan.zhihu.com/p/121602526
      this.$store.commit(
        'changeActiveTitle',
        'gerserver连接postgis发布wms地图服务，推荐3条芜湖路网数据(最短路径-紫色,道路类型-橙色,红绿灯-绿色)'
      )
    }
  }
}
